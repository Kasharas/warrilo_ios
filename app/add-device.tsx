...
import { validateDeviceForm } from '@/src/utils/validateDeviceForm';

console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
console.log('!!! ADD DEVICE SCREEN LOADED - LOGGING IS ACTIVE !!!');
console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
...
export default function AddDeviceScreen() {
  ...
  const handleSaveDevice = async () => {
    console.log('🔵 [STEP 1: UI] "Save Device" button pressed');
    console.log('   -> User ID:', user?.id);
    console.log('   -> Device Photo Uri Exists:', !!deviceImage);
    console.log('   -> Receipt Photo Uri Exists:', !!receiptImage);
    ...
    try {
      ...
      const validationFormData: AddDeviceFormData = {
        deviceName: deviceName.trim(),
        brand: brand || '',
      ...
      // Validate the form using the validation utility
      const validation = validateDeviceForm(validationFormData);

      if (!validation.isValid) {
        // Store validation errors in state
        setValidationErrors(validation.errors);

        // Show alert with all missing fields
      ...
      // Additional validation for required images
      if (!selectedDate) {
        Alert.alert('Error', 'Purchase date is required');
        setIsSubmitting(false);
        return;
      ...
      console.log('🔵 [STEP 2: PREP] background fileData ready');
      console.log('   -> Device URI (preview):', fileData.devicePhoto?.uri?.substring(0, 40));
      console.log('   -> Receipt URI (preview):', fileData.receiptPhoto?.uri?.substring(0, 40));

      // Upload the device - NOW AWAITED TO SHOW ERROR
      console.log('🔵 [STEP 3: DISPATCH] Calling uploadDevice service (AWAITED)...');

      const uploadResult = await uploadDevice(formData, fileData);

      console.log('🔵 [STEP 7: FINISH] Upload process result:', uploadResult);
      ...
      if (uploadResult.success) {
        ...
        setCompressionStatus('Upload failed: ' + uploadResult.error);
        Alert.alert('Upload Failed', 'The background upload failed with error: ' + uploadResult.error);
      }
