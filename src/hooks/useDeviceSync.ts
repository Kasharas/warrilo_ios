...
export const useDeviceSync = () => {
  ...
  const addDevice = useCallback(async (formData: DeviceFormData): Promise<{
    ...
    try {
      ...
      const compressedPhoto = formData.devicePhoto?.uri || null;
      const compressedReceipt = formData.receipt?.uri || null;
      
      console.log('Using pre-compressed images from add device screen');

      // 2. Create device object with pre-compressed images
  ...
  const updateDevice = useCallback(async (deviceId: string, formData: DeviceFormData): Promise<{
    ...
    try {
      ...
      console.log('Using pre-compressed images from edit device screen');

      // 2. Create updated device object with pre-compressed images
